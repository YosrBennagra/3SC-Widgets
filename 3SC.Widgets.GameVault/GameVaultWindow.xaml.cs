...
{
    private static readonly ILogger Log = Serilog.Log.ForContext<GameVaultWindow>();
    ...
    public GameVaultWindow() : this(Guid.Empty, 0, 0, 420, 340, false)
    {
    }

    public GameVaultWindow(Guid widgetInstanceId, double left, double top, double width, double height, bool isLocked)
    {
        InitializeComponent();
        ...
        InitializeWidgetWindow(
            ...
            new WidgetWindowParts(
                ...
                WidgetKey: "game-vault"));

        _viewModel = new GameVaultWidgetViewModel();
        DataContext = _viewModel;

        Log.Debug("GameVaultWindow created with InstanceId={InstanceId}", widgetInstanceId);
    }
