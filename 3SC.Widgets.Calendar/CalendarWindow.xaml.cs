...
using _3SC.Widgets.Calendar.Helpers;
using Serilog;
...
/// <summary>
/// Widget window for Calendar.
/// Displays month view with day selection and navigation.
/// </summary>
public partial class CalendarWindow : WidgetWindowBase
{
    private static readonly ILogger Log = Serilog.Log.ForContext<CalendarWindow>();

    private readonly CalendarWidgetViewModel _viewModel;
    ...
    public CalendarWindow() : this(Guid.Empty, 0, 0, 320, 280, false)
    {
    }

    public CalendarWindow(Guid widgetInstanceId, double left, double top, double width, double height, bool isLocked)
    {
        InitializeComponent();
        ...
        _viewModel = new CalendarWidgetViewModel();
        DataContext = _viewModel;

        Loaded += CalendarWindow_Loaded;

        Log.Debug("CalendarWindow created with InstanceId={InstanceId}", widgetInstanceId);
    }
    ...
    private void CalendarWindow_Loaded(object? sender, RoutedEventArgs e)
    {
        try
        {
            ...
            Log.Information("Calendar widget loaded at position ({Left}, {Top})", Left, Top);
        }
        catch (Exception ex)
        {
            Log.Error(ex, "Failed to initialize calendar widget window");
        }
    }
    ...
    protected override void OnClosing(CancelEventArgs e)
    {
        ...
        {
            _viewModel?.Dispose();
            Log.Information("Calendar widget closing");
        }
        catch (Exception ex)
        {
            Log.Error(ex, "Error during calendar widget cleanup");
        }
    ...
    {
        // Don't start dragging if clicking on resize handles, buttons, or scrollviewer
        while (source != null)
        {
