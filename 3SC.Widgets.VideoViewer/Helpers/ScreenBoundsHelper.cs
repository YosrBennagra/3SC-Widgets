...
public static class ScreenBoundsHelper
{
    public static Point ConstrainToScreenBounds(int x, int y, int width, int height)
    {
        ...
        foreach (var screen in screens)
        {
            ...
            if (distance < minDistance) { minDistance = distance; targetScreen = screen; }
        }

        var bounds = targetScreen.WorkingArea;
        return new Point(Math.Max(bounds.Left, Math.Min(x, bounds.Right - width)), Math.Max(bounds.Top, Math.Min(y, bounds.Bottom - height)));
    }
}
